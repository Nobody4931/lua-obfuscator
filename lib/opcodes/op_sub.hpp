...
class vop_sub_1_t : public vopcode_t {
public:
	...
	std::string string() override {
		return R"(local KeyB, KeyC = (function(...)
			local Keys = {...}
			for Idx = 1, #Keys do
				Keys[Idx] = Instr[Keys[Idx]] <= 255 and Stack[ Instr[Keys[Idx]] ] or Constants[ Instr[Keys[Idx]] - 256 ]
			end
			return Unpack(Keys)
		end)(2, 3)
...
class vop_sub_2_t : public vopcode_t {
public:
	bool valid( instruction_t& instruction ) override {
		return instruction.opcode == OP_SUB;
	}
...
public:
	vopcode_t* mutate( std::default_random_engine& rand_engine, bool base_case ) override {
		switch ( rand_engine() % ( base_case ? 2 : 6 ) ) {
			case 0: return new vop_sub_1_t();
			case 1: return new vop_sub_2_t();
			case 2: return new vop_sub_bc_1_t();
			case 3: return new vop_sub_bc_2_t();
			case 4: return new vop_sub_bc_3_t();
			case 5: return new vop_sub_bc_4_t();
		}
