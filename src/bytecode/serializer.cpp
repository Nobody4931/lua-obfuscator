	...
	for ( uint8_t i = 0; i < 3; ++i ) {
		...
		switch ( context.instr_order[i] ) {
			...
			case instr_order_t::ORD_OPCODE:
				write_byte( context.bytecode, context.opcode_map[ instruction.opcode ][ op_idx ].first, context.instr_xor_key );
				break;

			case instr_order_t::ORD_ENUM:
				write_byte( context.bytecode, context.enum_map[ type ], context.instr_xor_key );
				break;
			...
			case instr_order_t::ORD_FIELDS: // always 5 bytes
				switch ( type ) {
					...
					case instr_t::i_ABC:
						write_int16( context.bytecode, instruction.b, little_endian, context.instr_xor_key );
						write_int16( context.bytecode, instruction.c, little_endian, context.instr_xor_key );
						break;

					case instr_t::i_ABx:
						write_int32( context.bytecode, instruction.b, little_endian, context.instr_xor_key );
						break;

					case instr_t::i_AsBx:
						write_int32( context.bytecode, instruction.b + ( (1 << 17) - 1 ), little_endian, context.instr_xor_key );
						break;

				}

				write_byte( context.bytecode, instruction.a, context.instr_xor_key );
				break;
...
static inline void serialize_constant( obfuscation_context_t& context, constant_t& constant, bool little_endian ) {
	switch ( constant.type ) {
		...
		case const_t::K_BOOLEAN:
			write_byte( context.bytecode, *constant.data, context.const_xor_key );
			break;

		case const_t::K_NUMBER:
			write_double( context.bytecode, *reinterpret_cast<l_number*>( constant.data ), little_endian, context.const_xor_key );
			break;

		case const_t::K_STRING:
			write_some( context.bytecode, constant.data, reinterpret_cast<l_string*>( constant.data )->size, true, little_endian, context.const_xor_key );
			break;
	...
	for ( uint8_t i = 0; i < 5; ++i ) {
		switch ( context.chunk_order[i] ) {
			...
			case chunk_order_t::ORD_PARAM_CNT:
				write_byte( context.bytecode, chunk.parameter_cnt, context.param_xor_key );
				break;

			case chunk_order_t::ORD_UPVAL_CNT:
				write_byte( context.bytecode, chunk.upvalue_cnt, context.upval_xor_key );
				break;

			case chunk_order_t::ORD_INSTRUCTIONS:
				write_int32( context.bytecode, chunk.instruction_cnt, little_endian, context.chunk_xor_key );
			...
			case chunk_order_t::ORD_CONSTANTS: // security probably wtf
				write_int32( context.bytecode, chunk.constant_cnt, little_endian, context.chunk_xor_key );
				for ( l_int i = 0; i < chunk.constant_cnt; ++i )
					write_byte( context.bytecode, context.const_map[ chunk.constants[i].type ], context.const_xor_key );
				for ( l_int i = 0; i < chunk.constant_cnt; ++i )
					serialize_constant( context, chunk.constants[i], little_endian );
				break;

			case chunk_order_t::ORD_PROTOTYPES:
				write_int32( context.bytecode, chunk.function_cnt, little_endian, context.chunk_xor_key );
